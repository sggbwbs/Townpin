...
async function handleSendDigest(req, res) {
  ...
  // ?test=1 skips the 8am time-window check below -- for manually
  // triggering a REAL send (actual emails/pushes to actual current
  // subscribers) to verify a change without waiting for the next
  // natural 8am window, potentially a full day away. Requires
  // CRON_SECRET unconditionally, with NO graceful degrade if it's
  // unset -- unlike the auth check above (which intentionally allows
  // the normal cron path to work even without CRON_SECRET configured),
  // a known, guessable bypass parameter left wide open to anyone would
  // be a real risk, not a theoretical one, since real subscribers'
  // actual inboxes and phones are on the other end of a genuine send.
  const isTestTrigger = req.query.test === '1' && !!CRON_SECRET && req.headers['authorization'] === `Bearer ${CRON_SECRET}`;
