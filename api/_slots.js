// Shared slot-assignment helper. Previously only lived inside
...
// admin "grant free slot" and "move to another town" tools -- this is
...
// Picks N random empty slots in a town. Random (not just the first
// empty indices) so everyone doesn't pile into the same top-left corner
// of every board.
async function pickRandomEmptySlots(townId, count) {
  ...
  // can never be inserted again. Each slot's /pin/{id} page is meant to
  ...
  // caused a real bug: as slots churned (cancellations, downsizing,
  ...
  // once enough churn had happened in a town. See insertSlotsWithRetry
  // below for where that surfaced as "Could not find available slots
...
// Picking empty slots and then inserting them isn't one atomic
...
async function insertSlotsWithRetry(townId, count, buildRows, maxAttempts = 4) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const indices = await pickRandomEmptySlots(townId, count);
    if (indices.length < count) {
      return {
        error: indices.length > 0
          ? `Only ${indices.length} free slot(s) available in that town right now.`
          : 'No free slots available in that town right now.',
        rows: null
      };
    }

    const { data, error } = await supabase.from('slots').insert(buildRows(indices)).select();
  ...
  return { error: 'Could not find available slots after several attempts — please try again.', rows: null };
}

module.exports = { pickRandomEmptySlots, insertSlotsWithRetry };
