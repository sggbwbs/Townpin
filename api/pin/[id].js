...
module.exports = async (req, res) => {
  ...
  if (error || !square || square.status !== 'active' || square.flagged) {
    ...
    res.status(404).send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Not found — TownPin</title></head>
      <body style="font-family:sans-serif;text-align:center;padding:80px 20px;color:#333;">
        <h1>This square isn't claimed (yet)</h1>
        <p><a href="/">Back to TownPin</a></p>
      </body></html>`);
    return;
  }
  ...
  const title = `${escapeHtml(square.company_name)} — ${escapeHtml(townName)} | TownPin`;
