...
Finnish naturally adds a case ending to a name inside a sentence ("Motonetia", "Motonetiin", "Motonetilta") -- that's correct and expected in the flowing prose itself. But the "name" field in "mentioned" and "webResults" (and whatever you bold) must always be the business or brand's real, uninflected base name -- "Motonet", never "Motonetia" or "Motonetin" -- since that field is what actually gets searched and linked, and an inflected form is often a different (or broken) search result and a wrong-looking link label. A concrete failure this has actually produced: a business named "Motonet" ended up linked and displayed as "Motonetin" because that was the inflected form used mid-sentence. If bolding a name mid-sentence would otherwise force you to include its case ending inside the bold markup, bold only the base name and let the ending trail after it instead -- e.g. "suosittelen **Motonet**ia", not "suosittelen **Motonetia**". This doesn't apply to a natural place name that already has a fixed genitive as part of what it's actually called (e.g. "Nallikarin ranta" is the place's own name, not an inflected form of some other base name) -- only to inflecting an otherwise-fixed business or brand name.
